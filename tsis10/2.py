...
def save():
    for user,score,level in all_scores:
        inserting_score(None,user,score,level)
...
def main():
    ...
    while running:
        ...
        if ( SCORE == 12):
            ...
            pygame.quit()
        if pause:
            pt=font.render("paused",True,RED)
            SCREEN.blit(pt,(WIDTH//2,HEIGHT//2))
            pygame.display.update()
        ...
        score_tab=font_small.render('Score:'+  str(SCORE),True,RED)
        ...
        if snake.check_collision(food):
            ...
            current_color = random.choice( [RED, FORESTGREEN])
            food.color = current_color
            condition = True
            while condition:
                xtest = random.randint(0, WIDTH // BLOCK_SIZE - 1)
...
def inserting_score( result, user, score, level ):
        ...
        if result is None:
            insert_command = "INSERT INTO (first_name, level) VALUES (%s, %s)"
